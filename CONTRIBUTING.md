...
in action and review the output of the test workflows carefully.

Create pull requests for the `main` branch. Pull requests should contain a
single commit with a commit message that is a brief sentence with punctuation.
Include the text "(fixes #??)" at the end of the commit message so the pull
request will be associated with the relevant issue. Squash multiple commits
before creating the pull request or when making updates. (See [Git Tools -
Rewriting History][rewriting] for details.)
...
[issues]: https://github.com/DavidAnson/markdownlint-cli2-action/issues
